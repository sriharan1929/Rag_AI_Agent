...
echo [2/3] Starting Backend on Port 8000 (Global/Network) ...
start "ZIP-RAG Backend" "%~dp0start_backend.bat"
timeout /t 5 /nobreak >nul

echo [3/3] Starting Frontend on Port 3000 (Global/Network) ...
start "ZIP-RAG Frontend" "%~dp0start_frontend.bat"

echo.
echo Done! App will open at http://localhost:3000
echo (It is also accessible globally via your local IP address)
pause
